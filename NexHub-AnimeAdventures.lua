        ...
        while task.wait() do 
            if getgenv().autoabilities then
                if game.PlaceId ~= 8304191830 then
                    ...
                    for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
                        repeat task.wait() until v:WaitForChild("_stats")
                        if (v.Name == "erwin" or v.Name == "erwin:shiny" or v.Name == "erwin_school") and tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name and v["_stats"].upgrade.Value >= 3 then
